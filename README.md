...
rfars is package with a set of functions that makes easier to analyze the data from the US National Highway Traffic Safety Administration's Fatality Analysis Reporting System (FARS).
...
10. Finally, to use the continuous integration service of Travis, run `devtools::use_travis()` and activate the package's repository in your Travis account; then, add, commit, and push your changes in Github; then, include the Travis badge into the README file.
