...
6.  Fill the [DESCRIPTION](DESCRIPTION) file with the information of your package.
7.  Document the code in the [R](./R) folder using roxygen2-style documentation, and execute `devtools::document()` (or press Ctrl/Cmd + Shift + D in RStudio) in order to create the `.Rd` files. Later, press the RStudio's `Build & Reload` button and review the help files with `?`.
